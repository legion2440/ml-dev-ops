...
- Public entrypoint: `client/inference_client.py` (`implemented`).
- Root: `client`.
...
- Public entrypoint: `client/logging/writer.py` (`implemented`).
- Root: `client/logging`.
- Tests: schema stability, append behavior, path sanitization, error records, and
  CSV row parity.
...
- Inputs: a generated shared model-pair contract, deterministic binary input, batch
  sizes, concurrency, warmup, request count, Triton runtime metadata, and
  Windows-host process-attributed NVIDIA telemetry.
- Outputs: indexed raw measurements, aggregate CSV files, a comparison report, and
  hashed runtime evidence.
- Public entrypoint: `benchmarks/run_benchmark.py` (`implemented`).
- Root: `benchmarks`.
- Tests: aggregation, percent-change calculations, boundary telemetry,
  contamination classification, replacement protection, errors, and report
  generation.
...
- Outputs: complete serving ModelConfig data, a generated client-facing model
  contract, and a generated benchmark-pair projection without artifact paths.
- Public entrypoints: `shared/triton_model_config.py` and generated
  `shared/client-model-contracts.json` and `shared/benchmark-model-pair.json`
  (`implemented`).
- Root: `shared`.
...
- Client-facing metadata is generated by model preparation into shared contracts;
  the runtime client never reads the model spec, manifest, or artifact paths.
- Benchmarking may use the public client transport, shared contracts, and Triton
  endpoints. It does not read the model repository or inference logs.
- Deployment owns the profile-only SDK benchmark runner; the repository is mounted
  read-only and only ignored benchmark cache is writable during measurement.
- The Windows host owns the benchmark environment observer. Marker/ack files in the
  ignored cache establish host sequence boundaries without comparing host and
  container monotonic clocks.
...
```mermaid
flowchart LR
    Manifest["Generated model manifest"] --> Projection["Model preparation projection"]
    Labels["Tracked label files"] --> Projection
    Projection --> Contract["Generated client model contract"]
    Contract --> Client["Runtime image client"]
    Client --> Triton["Triton HTTP or gRPC"]
    Client --> JSONL["Append-only JSONL events"]
    JSONL --> CSV["Derived CSV export"]
```

The projection deliberately excludes artifact paths, source weights, TensorRT plan
locations, and other model-repository implementation details.
