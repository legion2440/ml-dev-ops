...
| Step 6 evidence integrity | Runtime fingerprint `82e105…`, per-file runtime source hashes, and behavioral PA/aggregation/guard probes in the narrow benchmark semantic projection | `python scripts/validate_benchmark_evidence.py --check`; `--historical-only` | Historical measurement integrity is immutable; critical code behavior drift fails compatibility while Grafana/Prometheus and validator evolution remain provenance-only |
